...
class QRAppTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.qr_code = QRCode.objects.create(
            name="Test QR Code", text_content="unique-qr-string-123", points=50
        )
    ...
    def test_authenticated_user_can_scan_qr_code(self):
        ...
        response = self.client.post(
            self.scan_url, {"text_content": self.qr_code.text_content}
        )
        ...
        self.assertTrue(
            UserScannedQR.objects.filter(user=self.user, qr_code=self.qr_code).exists()
        )
    ...
    def test_get_rewards_history(self):
        ...
        self.assertEqual(
            response.data["scan_history"][0]["qr_code"]["name"], self.qr_code.name
        )
    ...
    def test_regular_user_cannot_create_qrcode(self):
        ...
        self.client.force_authenticate(user=self.user)
        data = {"name": "Attempted QR", "text_content": "hacker-string", "points": 1000}
        response = self.client.post(self.qrcode_list_url, data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
